...
async def _setup_sqlite_schema():
    ...
    import feature_store_api.core.config as config
    from feature_store_api.core.db import engine

    # Clear cached settings between tests (defensive)
    ...
    async with engine().begin() as conn:
        ...
        # Two rows for same user: latest should win.
        await conn.execute(
            text(
                """
...
def test_materialize_and_get_online(_setup_sqlite_schema):
    ...
    # values stored as strings (Redis-like behavior)
    assert feats["age"] == "29"
